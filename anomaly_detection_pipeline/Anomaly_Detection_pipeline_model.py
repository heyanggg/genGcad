...
def Anomaly_detection(dataset, new_env, thres, method, model, t):
    model_name = f"check_model/best_{dataset}_{model}_{method}.pth"
    vocab_size = vocab_dic[dataset]
    epochs = 15
    seq_len = 10
    data_file = f'synthetic_data/{dataset}_{new_env}_generation_{method}_th={thres}_{model}_seq_filter_true.pkl'
    if new_env == 'multiple':
        vld_file = data_file
    ...
    test_file3 = f"test/{dataset}/{new_env}/test.pkl"

    setup_seed(2024)
    train(new_env, vocab_size, epochs, train_file, model_name, seq_len)
    threshold = find_threshold(new_env, vocab_size, vld_file, model_name, seq_len, percentage=t)
    recall, precision, accuracy, f1_score = evaluate(new_env, vocab_size, test_file1, test_file3, model_name, seq_len,
                                                     threshold=threshold)
