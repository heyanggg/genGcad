...
from original_gen_cache import reuse_original_gen_cache
...
def get_args_parser():
    ...
    parser.add_argument(
        "--reuse-original-gen-selection-dir",
        default=None,
        help=(
            "Directory containing a manifest-verified, independently generated "
            "original SmartGen pre-LLM SSC cache. Only valid with --gcad-mode off"
        ),
    )
...
def prepare_gcad_stage(args):
    ...
    if getattr(args, "reuse_original_gen_selection_dir", None):
        raise ValueError(
            "--reuse-original-gen-selection-dir is only valid with --gcad-mode off"
        )
...
def run_generation(args, config):
    ...
    try:
        ...
        if args.method == "SPPC":
            if args.reuse_original_gen_selection_dir:
                if args.gcad_mode != "off":
                    raise ValueError(
                        "--reuse-original-gen-selection-dir is only valid with "
                        "--gcad-mode off"
                    )
                compression = reuse_original_gen_cache(
                    Path(args.reuse_original_gen_selection_dir),
                    dataset=args.dataset,
                    original_environment=args.ori_env,
                    target_environment=args.new_env,
                    threshold=args.threshold,
                ...
                run.update(compression=compression)
            elif args.reuse_sppc_selection_dir:
