...
def find_threshold(
    ...
    if scores_path is not None:
        np.savez_compressed(
            ...
            percentile=np.asarray([percentage], dtype=np.float64),
...
def evaluate(
    ...
    print('Finished Test')
    metrics = (TP, TN, FP, FN, FPR, FNR, recall, precision, accuracy, f1_score)
    if scores_path is not None:
        np.savez_compressed(
            scores_path,
...
def Anomaly_detection(
    ...
    validation_scores_path = run_dir / "validation_scores.npz"
    continuous_scores_path = run_dir / "continuous_scores.npz"
    threshold = find_threshold(
        ...
        scores_path=validation_scores_path,
    )
    evaluation = evaluate(
        new_env,
        vocab_size,
        ...
        seq_len,
        threshold=threshold,
        scores_path=continuous_scores_path,
        detailed=True,
    )
    if isinstance(evaluation, dict):
        metrics = evaluation["threshold_metrics"]
        ranking_metrics = {
            "auroc": _json_native(evaluation["auroc"]),
            "auprc": _json_native(evaluation["auprc"]),
        }
    else:  # Backward-compatible with injected/legacy evaluators.
        metrics = evaluation
        ranking_metrics = {"auroc": None, "auprc": None}
    metric_names = (
    ...
    result = {
        ...
        "continuous_scores_path": str(continuous_scores_path),
