...
- 3 组 GCAD-on/off 已按“等训练样本、训练/验证不重叠、3 个检测器 seed、同时报告 F1/AUROC/AUPRC”完成复评。结果显示：SP single→multiple 明显支持 GCAD；US daytime→night 基本持平且任务过易；US winter→spring 的 F1 略支持 on，但 AUROC/AUPRC 反而支持 off。GCAD 收益具有任务依赖性，当前不能声称它在所有任务上稳定有效。
...
### 6.3 下一优先项：LLM 独立生成重复

当前复评重复的是检测器训练，不是生成过程。每个任务、每种模式仍只有一份 LLM 生成数据；因此下一步应对上述 3 个任务分别再执行两次 on 和两次 off，共新增 **12 个完整生成运行**，然后对每次生成使用相同的稳健检测协议。
...
1. 三组严格配对的 GCAD-off 生成和稳健检测器复评均已完成。
2. 下一步新增 12 个 on/off 独立生成重复，形成每种设置 3 次生成的均值 ± 标准差。
3. 生成重复后，优先选择持续出现正向信号的任务做 shuffled-GCAD；不预先假定一定是 US spring。
