...
GCAD v2 tightened the source side further: all audit, representation, split, and prediction inputs are the FR winter source `trn.pkl`/`split_trn.pkl` plus static dictionaries. The source prediction gate failed, so relation extraction, GSS fusion, B5, and all target-data stages remained unreachable. Every v2 report records `target_behavior_used=false`.
